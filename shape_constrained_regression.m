function [p,x] = shape_constrained_regression(algo,degree,features,...
                 response, varargin)
%% Description
% Outputs
...
switch algo
    case "monotone"
        ...
        [p,x] = monotone_regression(degree,features,response,...
                                    arg_struct.monotone_profile);
    case "bounded_derivative"
        % fit to the training data
        [p,x] = bounded_derivative_regression(degree,features,response,...
                                              arg_struct.l_bound,...
                                              arg_struct.u_bound);                                       
    case "convex"
        % fit to the training data
        [p,x] = convex_regression(degree,features,response,...
                                  arg_struct.convex_sign);   
    case "monotone_convex"
        % fit to the training data
        [p,x] = monotone_convex_regression(degree,features,response,...
                                           arg_struct.monotone_profile, ...
                                           arg_struct.convex_sign);            
end
